...
const NotificationBar = () => (
  <div className="flex items-center gap-4 mb-3 px-4 py-3 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden shrink-0 h-[60px]">
    {/* Left Label */}
      ...
      {tabList.map((tab) => {
        ...
        return (
          <button
            ...
            className={`h-12 border rounded-xl text-[13px] font-bold transition-all duration-200 flex items-center justify-center gap-2 px-4 shadow-sm hover:scale-[1.02] active:scale-[0.98] ${
              isActive 
          ...
          >
            <div className="relative flex items-center">
               ...
               {tab === '预警通知' && (
                 <div className="absolute -top-1.5 -right-1.5 min-w-[14px] h-[14px] bg-red-500 rounded-full text-white text-[9px] flex items-center justify-center border border-white leading-none px-0.5">
                   9
                 </div>
               )}
            </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex items-center shadow-sm h-[60px] mb-3">
      <div className="flex items-center gap-4 px-6 flex-1 overflow-x-auto no-scrollbar">
        <div className={`flex items-center gap-3 shrink-0 sticky left-0 bg-white z-10 pr-4 ${isWorkDaily ? 'mr-2' : 'mr-8'}`}>
        ...
        <div className={`flex items-center ${isWorkDaily ? 'gap-4' : 'gap-16'} shrink-0`}>
          {data.map(([label, val, color]) => (
            <div key={label} className={`flex flex-row items-center whitespace-nowrap ${isWorkDaily ? 'gap-1.5' : 'gap-3'}`}>
